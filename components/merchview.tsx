...
export function MerchView({ merch }: { merch: Merch }) {
      ...
      <div className="flex  items-center justify-center relative p-2 md:p-20 cursor-pointer md:-mt-20">
        ...
        >
          <Card rotate={rotate} translate={translate} scale={scale}>
            <Image
              src={merch.frontImage}
              alt="hero"
              height={720}
              width={1400}
              className="mx-auto rounded-md grayscale group-hover:grayscale-0 transition duration-200 object-cover object-center h-full"
              draggable={false}
            />
          </Card>
        </div>
...
function Blog({ merch }: { merch: Merch }) {
  return (
    <GradientContainer className="md:my-20">
      <Container className="py-20 max-w-5xl mx-auto relative z-40">
        <div className="py-10 text-white">
          <div className="text-lg leading-relaxed">{merch.description}</div>
        </div>
      </Container>
    </GradientContainer>
  );
